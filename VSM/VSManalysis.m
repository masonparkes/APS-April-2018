...
[fr,mr]=interp2sizeVSM(Froom,Mroom,1001);
mr(1)=mr(2);
%%linear component
spread=5000;
weight=.05;
linearPart=tanh(fr/spread)*weight;
%noise
...
for number=196:205
    ...
    figure;
    hold on;
    plot(fr,zscore(manip));
    makePlot(number);
    legend('manipulated VSM data','APS data');
    xlabel('applied field (Oersted)');
